"""
react_agent.py
The ReAct agent that drives the adaptive interview.

ReAct and LangGraph are two different things, and this file exists to keep
them apart.

ReAct is the PATTERN (Yao et al., 2022): an explicit Thought, a model-chosen
Action, a real Observation returned from outside the model, and a loop back so
the next Thought is grounded in what was observed. It is an idea about how to
prompt a model, and it would work over plain API calls with no library at all.

LangGraph is the MACHINERY this file happens to use to express that pattern.
The whole of the ReAct loop is one line of it:

    graph.add_edge("tools", "agent")

Everything else here is the agent's own substance: the state it carries, the
five tools it may call, the briefing it is given, the fallback for when it
returns nothing, and the three budget checks the code applies to its decision
afterwards.

The distinction matters in one practical way. The agent has the authority of
an OPINION, not of a HAND: the tools only record a decision into a context
dict, and it is `run_answer_cycle` below — ordinary Python, not the graph —
that reads that decision, overrides it when it breaks a budget, and throws the
resulting question away if it fails the Answerability gate.

The separate pipeline in graph_engine.py is LangGraph without ReAct: there the
path is fixed and the code decides. Here the model decides. The two are
deliberately not in the same file.
"""
